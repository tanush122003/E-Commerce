...
const Kids = () => 
{
 ...
 const setThePageUp = ()=>
 {
   const filterProducts = allKidsProducts.slice(0,TOTAL_ITEMS_PER_PAGE)
   setFilteredProducts(filterProducts)
   setTotalPages(Math.ceil(allKidsProducts.length/TOTAL_ITEMS_PER_PAGE))
 }
 ...
 const handlePageButtonClick = (pageNumber) => 
 {
   ...
   const filterProducts = allKidsProducts.slice(startOffset, endOffset);
   setFilteredProducts(filterProducts);
 };
  ...
  useEffect(()=>{
    if(filteredProducts && selectOption)
    {
      ...
      {
        console.log(shuffle(allKidsProducts),'shuffle womenPants')
  ...
  useEffect(()=>
  {
    setAllKidsProducts(products.filter(p=>p.category.slug==='kids' ))
  },[products])
  ...
  return (
    ...
    <section id='bgm' className="container">
     ...
     <h1 id='tagname' className="text-center">Kids</h1>
