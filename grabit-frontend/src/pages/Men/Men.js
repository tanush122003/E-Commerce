...
const Men = () => 
{
  ...
  const setThePageUp = ()=>
  {
    const filterProducts = allMenProducts.slice(0,TOTAL_ITEMS_PER_PAGE)
    setFilteredProducts(filterProducts)
    setTotalPages(Math.ceil(allMenProducts.length/TOTAL_ITEMS_PER_PAGE))
  }
  ...
  const handlePageButtonClick = (pageNumber) => 
  {
   ...
   const filterProducts = allMenProducts.slice(startOffset, endOffset);
   setFilteredProducts(filterProducts);
  };
  ...
  useEffect(()=>
  {
    if(filteredProducts && selectOption)
    {
      ...
      {
        console.log(shuffle(hoodies),'shuffle womenPants')
  ...
  useEffect(()=>
  {
    setAllMenProducts(products.filter(p=>p.category.slug==='men-hoodies' || p.category.slug==='men-shirts' || p.category.slug==='men-pants' ))
  },[products])
  ...
  return (
    ...
    <section id='bgm' className="container">
     ...
     <h1 id='tagname' className="text-center">All Men Products</h1>
