...
const Hoodies = () => 
{
  ...
  const setThePageUp = ()=>
  {
    const filterProducts = hoodies.slice(0,TOTAL_ITEMS_PER_PAGE)
    setFilteredProducts(filterProducts)
    setTotalPages(Math.ceil(hoodies.length/TOTAL_ITEMS_PER_PAGE))
  }
  ...
  const handlePageButtonClick = (pageNumber) => 
  {
   ...
   const filterProducts = hoodies.slice(startOffset, endOffset);
   setFilteredProducts(filterProducts);
 };
  ...
  useEffect(()=>
  {
    setHoodies(products.filter(p=>p.category.slug==='men-hoodies'))
  },[products])
  ...
  return (
    ...
    <section id='bgm' className="container">
     ...
     <h1 id='tagname' className="text-center">Hoodies</h1>
