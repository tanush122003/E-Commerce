...
const Women = () => 
{
 ...
 const setThePageUp = ()=>
 {
   const filterProducts = pants.slice(0,TOTAL_ITEMS_PER_PAGE)
   setFilteredProducts(filterProducts)
   setTotalPages(Math.ceil(pants.length/TOTAL_ITEMS_PER_PAGE))
 }
 ...
 const handlePageButtonClick = (pageNumber) => 
 {
   ...
   const filterProducts = pants.slice(startOffset, endOffset);
   setFilteredProducts(filterProducts);
 };
  ...
  useEffect(()=>
  {
    if(filteredProducts && selectOption)
    {
      ...
      {
        console.log(shuffle(pants),'shuffle womenPants')
  ...
  useEffect(()=>
  {
    setPants(products.filter(p=> p.category.slug==='women-pants'))
  },[products])
  ...
  return (
   ...
   <section id='bgm' className="container">
     ...
     <h1 id='tagname' className="text-center">Pants</h1>
           ...
           <h4 className="card-title">{singleProduct.name}</h4>
            ...
            <p className="card-text text-center">{singleProduct.description}</p>
             ...
             <i id='addto' className="fa fa-cart-shopping" />Remove Item</Link>
              : 
              <Link id='addtocart' onClick={()=>dispatch({type:"ADD_TO_CART",payload:{product:singleProduct,addedBy:auth?.user?.email,totalledBy:auth?.user?.email}})} className="btn btn-primary justify-content-centero"> <i id='addto' className="fa fa-cart-shopping" /> Add To Cart</Link>  
            }
