...
const Hoodies = () => 
{
 ...
 const setThePageUp = ()=>
 {
   const filterProducts = dresses.slice(0,TOTAL_ITEMS_PER_PAGE)
   setFilteredProducts(filterProducts)
   setTotalPages(Math.ceil(dresses.length/TOTAL_ITEMS_PER_PAGE))
 }
 ...
 const handlePageButtonClick = (pageNumber) => 
 {
   ...
   const filterProducts = dresses.slice(startOffset, endOffset);
   setFilteredProducts(filterProducts);
 };
  ...
  useEffect(()=>
  {
    if(filteredProducts && selectOption)
    {
      ...
      {
        console.log(shuffle(dresses),'shuffle womenPants')
  ...
  useEffect(()=>
  {
    setDresses(products.filter(p=>p.category.slug==='women-dresses'))
  },[products])
  ...
  return (
   ...
   <section id='bgm' className="container">
     ...
     <h1 id='tagname' className="text-center">Dresses</h1>
